...
std::string makeHttpResponse(std::string const& status, std::string const& body) {
    return
        "HTTP/1.1 " + status + "\r\n"
...
void TcpServer::start() {
    ...
    while (true) {
        ...
        if (bytes_received > 0) {
            ...
            bool is_http =
                message.find("GET ") == 0 || // == 0, msg starts with GET, POST, etc.
                message.find("POST ") == 0 ||
                message.find("PUT ") == 0 ||
            ...
            if (is_http) {
                ...
                std::size_t body_position = message.find("\r\n\r\n");

                // If "\r\n\r\n" was found, there is a body after the HTTP headers
                if (body_position != std::string::npos) { // npos = not found 
            ...
            try {
                ...
                else if (method == "PUT" && path == "/patients") {
                ...
                // DELETE /patients
                // Body: 2
                else if (method == "DELETE" && path == "/patients") {
                ...
                // Body: 1 kevin admin
                    ...
                    if (data >> id >> username >> role) {

                        db.addUser(id, username, role);
                ...
                // GET /studies
                else if (method == "GET" && path == "/studies") {
                ...
                // POST /studies
                // Body: 10 1 Brain_MRI
                else if (method == "POST" && path == "/studies") {
                    ...
                    if (data >> id >> patient_id >> description) {

                        db.addStudy(id, patient_id, description);
                ...
                // Body: 10 1 Updated_Brain_MRI
                else if (method == "PUT" && path == "/studies") {
                    ...
                    if (data >> id >> patient_id >> description) {
                        ...
                        db.updateStudy(id, patient_id, description);
                ...
                // DELETE /studies
                // Body: 10
                else if (method == "DELETE" && path == "/studies") {
